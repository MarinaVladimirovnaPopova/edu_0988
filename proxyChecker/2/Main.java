...
public class Main {
    public static void main(String[] args) {
        try {
            FileInputStream fis = new FileInputStream("C://Users/Sergey/Desktop/java/ip.txt"); // Создаём поток ввода
            int i;
            String resultIp = "";
            while ((i=fis.read())!=-1){ // Читаем из файла, в конце фала вернётся -1
                if(i == 13) continue; // Символ возврата каретки
                    ...
                    Thread runnableProxyChecker = new Thread(new RunnableProxyChecker(ip, port));
                    runnableProxyChecker.start();
                    resultIp = "";
...
class RunnableProxyChecker implements Runnable{
    private String ip;
    private int port;
    RunnableProxyChecker(String ip, int port){
        this.ip = ip;
        this.port = port;
    }
